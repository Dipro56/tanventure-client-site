  ...
  const filteredPackages = useMemo(() => {
    ...
    return packages.filter((pkg) => {
      ...
      const searchableFields = [
        ...
        pkg.location,
        pkg.description,
        pkg.category, // if you have category field
        pkg.tags?.join(' '), // if you have tags array
        ...
        <div className="mb-6 text-center">
          <p className="text-gray-600">
            {filteredPackages.length > 0 
              ? `Found ${filteredPackages.length} package(s) matching "${searchValue}"`
              : `No packages found matching "${searchValue}"`
            }
          </p>
        </div>
      )}
